...
private
  ...
  def data
    array.map do |auxiliary|
      [
        ...
        link_to(content_tag(:span, '', class: "glyphicon glyphicon-#{img_status(auxiliary.status)}") + title_status(auxiliary.status), change_status_auxiliary_path(auxiliary), class: 'btn btn-warning btn-sm', remote: true)
      ]
    end
  end
  ...
  def per_page
    params[:iDisplayLength].to_i < 0 ? Auxiliary.count : params[:iDisplayLength].to_i
  end

...
private
  ...
  def fetch_array
    ...
    if params[:sSearch].present?
      array = array.where("assets.code like :search or description like :search or users.name like :search or auxiliaries.name like :search", search: "%#{params[:sSearch]}%")
    end
    array
  end
  ...
  def sort_column
    columns = %w[assets.code description users.name auxiliaries.name]
